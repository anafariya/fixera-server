...
let testBookingId: mongoose.Types.ObjectId;
...
async function cleanupTestFixtures() {
  ...
  await Project.deleteById?.(testProjectId) ?? await Project.findByIdAndDelete(testProjectId);
...
async function testBookingModelStoresDiscount() {
  ...
  assert(!!reloaded?.discount, 'Discount stored on booking');
  assertClose(reloaded!.discount!.totalDiscount, discount.totalDiscount, `Stored totalDiscount is €${discount.totalDiscount}`);
  assertClose(reloaded!.discount!.discountedAmount, discount.discountedAmount, `Stored discountedAmount is €${discount.discountedAmount}`);
  assert(reloaded!.discount!.loyaltyDiscount.tierName === 'Gold', 'Stored loyalty tier name is Gold');
  assertClose(reloaded!.discount!.loyaltyDiscount.amount, discount.loyaltyDiscount.amount, `Stored loyalty amount is €${discount.loyaltyDiscount.amount}`);
  assertClose(reloaded!.discount!.repeatBuyerDiscount.amount, discount.repeatBuyerDiscount.amount, `Stored repeat amount is €${discount.repeatBuyerDiscount.amount}`);
}
